...
export class M2RibbonEmitter implements IDisposable {
    ...
    initialize() {
        ...
        this.edges = new Array(newEdgesCount);
        ...
        this.tmpDU = (this.texBox.maxX - this.texBox.minX) / cols;
        this.tmpDV = (this.texBox.maxY - this.texBox.minY) / rows;
        this.rows = rows;
        this.cols = cols;
        ...
        this.texSlotBox = {
            ...
            maxX: this.texBox.minX + this.tmpDU,
            maxY: this.texBox.minY + this.tmpDV
        }
        this.heightAbove = 10;
        this.heightBelow = 10;
    ...
    update(deltaTime: number): void {
        ...
        // TODO: What does this do? Set read to last updated?
        while (this.readPos != this.writePos && (deltaTime + this.edges[this.readPos]) <= this.edgeLifetime) {
            this.readPos = this.advance(this.readPos, 1);
        }
        ...
        if (this.visible && this.initializedPosition) {
            ...
            if (!noNewEdges && newEdges > -1) {
                ...
                while (newEdges !== -1) {
                    ...
                    this.vertices[2 * this.writePos].color = this.color;
                    this.vertices[2 * this.writePos + 1].color = this.color;
        ...
        let currentEdge = this.readPos;
        while (currentEdge != this.writePos) {
            const vertexA = this.vertices[2 * currentEdge];
            const vertexB = this.vertices[2 * currentEdge + 1];

            const gravVal = this.gravity * 2 * this.edges[currentEdge] * deltaTime + deltaTime * this.gravity * deltaTime;
            ...
            this.edges[currentEdge] = deltaTime + this.edges[currentEdge];

            const nextCol = this.tmpDU * this.edges[currentEdge] * this.invLifespan + this.texSlotBox.minX;
            ...
            currentEdge = nextEdge - this.edges.length;
            if (this.edges.length > nextEdge) {
    ...
    draw(): void {
        ...
        for(let i = 0; i < this.m2data.textureIndices.length; i++) {
            ...
            if (blendMode !== GxBlend.GxBlend_Opaque) {
                ...
                blendMode = GxBlend.GxBlend_Opaque;
            }
            ...
            const count = this.writePos > this.readPos 
                ? 2 * (this.writePos - this.readPos) + 2 
                : 2 * (this.edges.length + this.writePos - this.readPos) + 2;
            batchRequest.drawIndexedTriangleStrip(2 * this.readPos * 2, count);
    ...
    private updateAnimatedProps() {
        ...
        if (this.texSlot != texSlot) {
            ...
            const minX = texSlotMod * this.tmpDU + this.texBox.minX;
            ...
            let minY = texSlotDiv * this.tmpDV + this.texBox.minY;
            this.texSlotBox.minY = minY;
            this.texSlotBox.maxX = minX + this.tmpDU;
            this.texSlotBox.maxY = minY + this.tmpDV;
        }
    ...
    private advance(currentIndex: number, steps: number) {
        ...
        let maxEdges = this.edges.length;
    ...
    private interpEdge(age: number, t: number, advance: number) {
        const firstVertex = this.vertices[2 * this.writePos];
        const secondVertex = this.vertices[2 * this.writePos + 1];
        ...
        this.edges[this.writePos] = age;
        this.writePos = this.writePos + advance;
        if (this.writePos >= this.edges.length) {
            this.writePos -= this.edges.length;
        }
    }
    ...
    private setupGraphics() {
        ...
        let indexBufferData = new Uint16Array(4 * this.edges.length);
        for (let i = 0; i < indexBufferData.length; i++) {
            indexBufferData[i] = i % (2 * this.edges.length);
        }
